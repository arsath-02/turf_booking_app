...
mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/turfdb', { 
...
app.post('/register', async (req, res, next) => {
    ...
    try {
        await newUser.save();
        const newToken = jwt.sign({ email: email, role: role }, process.env.JWT_SECRET);
        res.status(200).json({
            success: true,
            data: {
                token: newToken,
...
app.post('/communities', authenticateJWT, async (req, res, next) => {
    const { name } = req.body;
    const newCommunity = new Community({ name, admin: req.user.userid });
    ...
    try {
        ...
        res.status(400).send('Error creating community');
        next(err);
    }
});
